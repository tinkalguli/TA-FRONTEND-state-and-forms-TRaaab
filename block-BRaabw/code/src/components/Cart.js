...
export default function Cart(props) {
  if(props.products.length) {
    var totalCount = props.products.map(item => item.count)
    .reduce((a, c) => a + c, 0);

    var totalPrice = props.products.map(item => item.totalPrice)
      .reduce((a, c) => a + c, 0).toFixed(2);
    
    var installments = props.products.map(item => item.installments)
      .sort((a, b) => b - a)[0];
  }
    ...
    <aside className={props.isCartVisible ? "cart" : "cart cart-disable"}>
      {
        ...
        : <button
            ...
            <div className="bag-img-div">
              ...
              <p className="total-count">{props.products.length ? totalCount : "0"}</p>
            </div>
          </button>
      }
